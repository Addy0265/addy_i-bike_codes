...
char data[5] , reading ;
int negative = 0, index;
int currentangle=0;
int imu_yaw , prev_yaw;
...
void reach_target_angle()
{
if (inputangle < 2)
{gotoAngle(0);}
else{
 
  gotoAngle(inputangle);
  
 // delay(100);
  
  imu_yaw = get_angle();
  
   currentangle= imu_yaw - ref_angle;
   Serial.print("    ");
   Serial.print("current angle:");
   Serial.print(currentangle);
   
   inputangle = goal -  currentangle;  
}
}
...
float get_angle() {
 ...
 for( index=0,reading=Serial1.read();reading != '#';index++)
