...
#define MAX_RESP_TIME_MS 80      // timeout - max time to wait for low voltage drop (higher value increases measuring distance at the price of slower sampling)
...
void turn()
{
 ...
 {
   if(dist[0] < dist[1] && currentAngle > -40)
   {
     Serial.println("RIGHT");
     changeAngle(1);
   }
   else if(dist[0] > dist[1] && currentAngle < 40)
   {
     Serial.println("LEFT");
     changeAngle(-1);
   }
