...
#define LEAST_COUNT_STEER 2 
#define LEAST_COUNT_HEADING 2

#define STEER_ANGLE_LIMIT 40
#define SONAR_ANGLE_LIMIT 25
#define HEADING_ANGLE_LIMIT 12
...
void driveMotor(int torque)
{
  ...
  {
     //setPin(BRAKEPIN);    // DANGEROUS THING - UNCOMMENT AT YOUR OWN RISK
  } 
...
void planPath()
{
  ...
  {
    if(sonarOut == RIGHT && abs(steerAngle) < SONAR_ANGLE_LIMIT && abs(steerAngle) != 1000) 
    {
       resetPin(STOP_DRIVE); 
       gotoAngle(20);
    }
    else if(sonarOut == LEFT && abs(steerAngle) < SONAR_ANGLE_LIMIT && abs(steerAngle) != 1000)
    {
       resetPin(STOP_DRIVE);
       gotoAngle(-20);
    }
...
void gotoAngle(int goal)
{
  ...
  {
    // Stop motor
    Serial.print(" Angle limit exceeded ");
    driveMotor(0);
  }
}
